...
namespace Trivial.Utilities
{
    public static class ResultExtensions
    {
        ...
        public static Task<T> ValueOr<T>(this Result<Task<T>> M, T Default) =>
            M.HasValue ? M.Value : Task.FromResult(Default);
        ...
        public static Result<T2> Bind<T1, T2>(this Maybe<T1> Value, Func<T1, Result<T2>> Func) =>
            Value.HasValue ? Func(Value.Value) : new NullReferenceException("Maybe<T> was null when binding to Result<T>");

        public static async Task<Result<T2>> Bind<T1, T2>(this Maybe<T1> Value, Func<T1, Task<Result<T2>>> Func) =>
            Value.HasValue ? (await Func(Value.Value)) : new NullReferenceException("Maybe<T> was null when binding to Result<T>");
    }
}
