...
public static class Try
{
    ...
    public static async Task<Result<Unit>> InvokeAsync<T>(Func<Task> Func)
    {
        try
        {
            await Func();
    ...
    public static async Task<Result<T>> InvokeAsync<T, T2>(Func<T2, Task<T>> Func, T2 Val)
    {
        try
        {
            return new Result<T>(await Func(Val));
        }
        catch(Exception t_E)
        {
            return new Result<T>(t_E);
